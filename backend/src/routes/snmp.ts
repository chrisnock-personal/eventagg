...
router.get("/sources", async (_req, res, next) => {
  try {
    const sources = await query(
      `SELECT * FROM snmp_trap_sources ORDER BY last_seen DESC NULLS LAST`
    );
    res.json(sources);
  } catch (err) { next(err); }
});
...
router.post("/sources", requireRole("editor", "admin"), async (req, res, next) => {
  try {
    ...
    const row = await queryOne(
      `INSERT INTO snmp_trap_sources (name, agent_addr, community, description)
       VALUES ($1,$2,$3,$4) RETURNING *`,
      [body.name, body.agentAddr, body.community, body.description ?? null]
    );
...
router.put("/sources/:id", requireRole("editor", "admin"), async (req, res, next) => {
  try {
    ...
    const row = await queryOne(
      ...
      [body.name ?? null, body.community ?? null, body.description ?? null, body.isActive ?? null, req.params.id]
    );
...
router.delete("/sources/:id", requireRole("admin"), async (req, res, next) => {
  try {
    await query(`DELETE FROM snmp_trap_sources WHERE id = $1`, [req.params.id]);
    res.status(204).send();
  } catch (err) { next(err); }
});
  ...
  try {
    ...
    const row = await queryOne(
      `INSERT INTO snmp_routing_rules
         (priority, match_community, match_agent, match_trap_oid, policy_id, key_field)
       VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
      [body.priority, body.matchCommunity ?? null, body.matchAgent ?? null, body.matchTrapOid ?? null, body.policyId, body.keyField]
    );
  ...
  try {
    ...
    const row = await queryOne(
      ...
      [body.priority ?? null, body.matchCommunity ?? null, body.matchAgent ?? null,
       ...
       body.isActive ?? null, req.params.id]
    );
...
router.delete("/rules/:id", requireRole("admin"), async (req, res, next) => {
  try {
    await query(`DELETE FROM snmp_routing_rules WHERE id = $1`, [req.params.id]);
    invalidateRoutingCache();
    res.status(204).send();
  } catch (err) { next(err); }
});
...
router.get("/log", async (req, res, next) => {
  try {
    ...
    const rows = await query(
      `SELECT * FROM snmp_trap_log ORDER BY received_at DESC LIMIT $1`,
      [limit]
    );
    res.json(rows);
  } catch (err) { next(err); }
